...
The "Language for swapping" combobox contains also a "Last detected source language" item. If you have source language configured to "Automatically detect language"  and you swap via script between source and target languages, the addon will use the last detected source language as new target language.

So, imagine this situation: you usually translate into English (your main language) and use  the "auto" source language to translate from various source languages. Let's say you lastly translated from italian to english to read an italian document. You want then to write in italian. If you already have previously selected "Last detected source language" and you swap source and target language via script, you will be able to translate english into italian without accessing directly to the addon settings again.
